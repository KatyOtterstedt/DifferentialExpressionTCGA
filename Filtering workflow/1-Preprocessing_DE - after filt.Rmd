---
title: "1- DIFFERENTIAL EXPRESSION - PREPROCESSING- removal AFTER normalisation"
...
Scanning through the counts matrix, we observe the presence of miRNA, lncRNA and another type of short, noncoding RNAs: YRNAs. Given there's no stablished way to deal with them properly, we tried two different workflows: removing them before normalisation of the data, to reduce noise among the values, or remove them afterwards, to keep the library size untouched during normalisation. The following is the latter.


## Removing Noncoding RNAs After Normalisation
We remove miRNA, lncRNA, and YRNA rows from the normalized matrix after normalization to reduce noise among the values.
```{r}
# Filter rows based on row names
counts_matrix_filt_aft <- counts_matrix_normalised[!grepl("^MIR|^LINC|^Y_", rownames(counts_matrix_normalised)), ]
# counts_matrix_filt_aft <- as.matrix(counts_matrix_filt_aft[2:nrow(counts_matrix_filt_aft),])
# counts_matrix_filt_aft <- apply(counts_matrix_filt_aft, 2, as.numeric)

# Check the dimensions of both matrices
dim(counts_matrix)
dim(counts_matrix_filt_aft)

counts_matrix_aft <- counts_matrix[which(row.names(counts_matrix) %in% row.names(counts_matrix_filt_aft)),]
...
COAD.surv.after <- clinical_COAD[,survival_columns]
table(is.na(COAD.surv.after$event_5y))
COAD.surv.after <- COAD.surv.after %>%
  filter(!is.na(event_5y))

RAC1_column <- which(rownames(counts_matrix_filt_aft) %in% c("RAC1"))

# Combining survival data with expression data of gene of interest
COAD.surv.after <- merge(COAD.surv.after, t(counts_matrix_filt_aft[c(1,RAC1_column),]), by = "bcr_patient_barcode")
# Converting the column to type integer 
COAD.surv.after$RAC1 <- as.integer(COAD.surv.after$RAC1)
table(is.na(COAD.surv.after$RAC1))
```
...
```{r message=FALSE, warning = FALSE}
boxplot(COAD.surv.after$RAC1, col = "palevioletred")

# Get mean and standard deviation
mean = mean(COAD.surv.after$RAC1)
std = sd(COAD.surv.after$RAC1)
...
outliers <- which(COAD.surv.after$RAC1 < Tmin | COAD.surv.after$RAC1>= Tmax)

# Assing NA values to outliers
COAD.surv.after$RAC1[outliers] <- NA
```
...
save(RAC1_column, counts_matrix_filt_aft, counts_matrix_aft, COAD.surv.after, file = "DE_after_filt_data.RData")
