---
title: "2-DE_Analysis - removal BEFORE normalisation"
...
load("~/Bioinfo Tesina/DE_before_filt_data.RData")
```
...
mean(COAD.surv.before$RAC1, na.rm = TRUE) 
sd(COAD.surv.before$RAC1, na.rm = TRUE)
median(COAD.surv.before$RAC1, na.rm = TRUE)
...
```{r}
#RAC1 is row 7245
groups <- ifelse(counts_matrix_normalised[RAC1_column, ] <= cutpoint, "low", "high")
table(groups)
```
...
y <- DGEList(counts=counts_matrix_filt_bef, group = groups)
